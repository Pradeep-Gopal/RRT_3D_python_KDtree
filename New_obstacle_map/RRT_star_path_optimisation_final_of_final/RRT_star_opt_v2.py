...
def obstacle_map2():
    ...
    ax2.bar3d(x, y, z, dx, dy, dz, color = color) 
    # ax2.bar3d(x, y, z, dx, dy, dz, color = color) 
...
while(goalcheck_circle(goal_x, goal_y, goal_z, seed[0], seed[1], seed[2]) == False):
    ...
    if ((seed not in visited_nodes) and not obstacle_check(seed[0], seed[1], seed[2])):
        ...
        if s not in visited_nodes:
            ...
            # ax2.plot3D((parent[0],s[0]), (parent[1], s[1]), (parent[2], s[2]), 'black')
...
while l < len(seed_list):
    ax2.plot3D((parent_list[l][0], seed_list[l][0]), (parent_list[l][1], seed_list[l][1]), (parent_list[l][2], seed_list[l][2]),  'black')
    l = l + 1
ax2.plot3D(x_path, y_path, z_path, "-r")
